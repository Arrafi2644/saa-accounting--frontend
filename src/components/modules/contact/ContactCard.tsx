...
export const ContactCard: React.FC<ContactCardProps> = ({ icon,backgroundColor, title, content, delay = 0 }) => {
  return (
    ...
    //   className="bg-white border hover:border-3 hover:border-[#AAE6FA] hover:shadow-[0_0_20px_#CBEEFB] transition-all duration-500 group rounded-lg p-6 hover:scale-102 "
    // >
    //   <div className="flex items-start gap-4">
       
    //      <div  className={`shrink-0 w-14 h-14 bg-[${backgroundColor}] rounded-lg group-hover:scale-110 transition-all duration-500 flex items-center justify-center`}>
    //       {icon}
    //      </div>
  ...
  <div className="flex items-start gap-4">
    ...
    <div className="flex-1 min-w-0">
      ...
      {Array.isArray(content) ? (
        ...
        <p className="text-gray-600 text-sm md:text-base break-words">{content}</p>
      )}
    </div>
  </div>
</motion.div>
